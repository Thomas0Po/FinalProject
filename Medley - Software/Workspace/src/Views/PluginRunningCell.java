...
public class PluginRunningCell extends ListCell<PluginsRunningStatus> {
		HBox hbox = new HBox();
		...
		@Override
		protected void updateItem(PluginsRunningStatus item, boolean empty) {
			super.updateItem(item, empty);
			setText(null); // No text in label of super class
			if (empty) {
				setGraphic(null);
			} else if(item != null) {
				label.setText(item != null ? item.getPluginName() : "Plugin");
				image = new Image(item.getPluginIcon());
				if (image == null){
